...
public class Test_ServerStart{
    ...
    public Test_ServerStart() {
        ...
        IoC.Resolve<ICommand>("IoC.Register", "CreateAndStartThread", (object[] args) => {
            return new ActionCommand( () => {threadsStartCount++;});
        }).Execute();

        IoC.Resolve<ICommand>("IoC.Register", "GetDictionary", (object[] args) => {
        ...
        IoC.Resolve<ICommand>("IoC.Register", "HardStopThreads", (object[] args) => {
            return new ActionCommand( () => {threadsStopCount++;});
        }).Execute();

        IoC.Resolve<ICommand>("IoC.Register", "ConsoleStartServer", (object[] args) => {
            start = true;
            return new EmptyCommand();
        }).Execute();
        IoC.Resolve<ICommand>("IoC.Register", "ConsoleStopServer", (object[] args) => {
            stop = true;
            return new EmptyCommand();
        }).Execute();
    ...
    public void CreateAndStartThreadTest()
    {
        ...
        var startServerCommand = new ConsoleStartServer(numOfThread);
    ...
    public void StopThreadTest()
    {
        ...
        var StopServerCommand = new ConsoleStopServer();
    ...
    public void ConsoleTest()
    {
        ...
        Assert.Contains("Запуск сервера...", output);
        IoC.Resolve<SpaceBattle.Lib.ICommand>("ConsoleStartServer", numOfThread).Execute();
        Assert.True(true == start);
        Assert.Contains("Все потоки успешно запущены", output);
        Assert.Contains("Остановка сервера...", output);
        IoC.Resolve<SpaceBattle.Lib.ICommand>("ConsoleStopServer").Execute();
        Assert.True(true == stop);
        Assert.Contains("Завершение программы", output);
    }
    [Fact]
    public void ExceptionHandlerStrategyTest()
    {
        ...
        var strategy = new ExceptionHandlerStrategy(command, exception);
